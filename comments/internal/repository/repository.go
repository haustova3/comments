...
func (rep *Repository) SaveComment(ctx context.Context, comment create_comment.Comment) (int64, error) {
	var err error
	commentID := int64(0)
...
func (rep *Repository) GetComments(ctx context.Context, productID int64) ([]get_comment.Comment, error) {
	r := New(rep.write)
	comments, err := r.GetCommentsByProduct(ctx, productID)
	if err != nil {
		return make([]get_comment.Comment, 0), err
	}
	res := make([]get_comment.Comment, len(comments))
	for i, val := range comments {
		res[i] = get_comment.Comment{
			ID:        val.ID,
			UserID:    val.UserID,
			ProductID: productID,
			Text:      val.Tx,
